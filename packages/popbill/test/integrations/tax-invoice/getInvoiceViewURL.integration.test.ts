...
describeTaxInvoiceIntegration('popbill tax-invoice integration: getInvoiceViewURL', () => {
  test('getInvoiceViewURL succeeds', async () => {
    const context = testkit.createTaxInvoiceMethodContext()
    const managementKey = await testkit.createIssuedInvoice(context, 'VIW')

    const response = await context.service.getInvoiceViewURL({
